...
void Palworld::UnrealOffsets::ApplyMemberVariableLayout()
{
    ...
    if (fs::exists(MemberVariableLayoutFile))
    {
        ...
        if (auto FileContents = FileBuffer.read_all(); !FileContents.empty())
        {
            Ini::Parser parser;
            parser.parse(FileContents);
            FileBuffer.close();

            // I wanted to just #include "MacroSetter.hpp" here, but it kept giving linker errors due to FArchive and FArchiveState so I had to do this.
            // FArchive and FArchiveState are not included in this, so if you're using this mod for another game, you'll have to figure out a way to include those.
